...
$$
\begin{gathered}
\Pi = \frac{t_{1}\dots t_{n}}{t_{n+1}} \\
\underline{\alpha_{1}\dots\alpha_{n}} \in (A \cup B)^{*}\\
\Theta = \begin{pmatrix}
x_{1} & \dots & x_{n} \\
\beta_{1} &  & \beta_{n}
\end{pmatrix}\quad \forall i=\overline{1,n}\quad (\alpha_{i}=t_{i}\Theta) \\
\\
\exists \Theta\qquad  
\underline{\alpha_{n+1} = t_{n+1}\cdot \Theta}
\end{gathered}
$$
$$
\begin{gathered}
\mathfrak{W} = \alpha_{1}, \alpha_{2}, \alpha_{3}, \dots, \underline{\underline{\alpha_{i}}}, \dots 
\\
\underline{\alpha_{i}}
\end{gathered}
$$
# Задачи
Пусть $P = (A,B,V,\prod)$
Задачей к этой системе является $\underline{t?}$ 
Решением задачи называется всякое слово $\underline{\underline{\alpha}}$ выводимое в $\underline{P}$, которые является применением $t$

## Замечание
постановка задачи понимается, как описание задачи, в которой неизвестные символы переменных обозначают ...
Тогда решение задачи получается заменой символов переменных на конкретные слова.

В основе механизма решения задач системой Поста, лежит специальный алгоритм, который последовательно строит все слова, выводимые в таких системах

### Механизм вывода
Такой механизм реализует перебор разных выводов в системе Поста в порядке возрастания их длин.

---
Рассмотрим пример схемы функционирования СП 
Строим все выводимые слова.
Пусть задано $P=(A,B,V,\Pi)$, где
$V = \{x_{1}, \dots, x_{n}\}$
$\Pi= \underbrace{ \pi_{1}, \dots, \pi_{r} }_{ \text{аксиомы} }, \underbrace{ \pi_{r+1}, \dots, \pi_{m} }_{ \text{остальные продукции} }$
$|A\cup B| \geq 2$

$\underline{P}(\Omega_{p})$
Будем строить множества, моделируя выводы в этой системе *по шагам*.
На шаге с номером $\underline{\underline{k}}$ перебираются все выводы длины $k$ в такой системе, в которых применяются продукции, используются подстановки $\beta_{i}=(|\beta_{i}| \leq k)$
$$
\Theta = 
\begin{pmatrix}
x_{1} & \dots & x_{n} \\
...
\end{pmatrix}
$$
Множество выводов будем строить с помощью конструкции дерева, в котором следующий ярус составляют вершины, размеченные выводимыми словами, образующими все возможные продолжения отдельно взятых выводов.

---
$$
k = 1
$$
На этом шаге строятся все возможные выводы длины 1, в которых производятся замены переменных на слова и на которых ... - 1
$$
\begin{gathered}
\Theta = \begin{pmatrix}
x_{1} & \dots & x_{n} \\
